...
function displayPics(){
  ...
  // PUT YOUR RESPONSE IN THIS COMMENT : We declared rando inside displayPics function, so we cant acssesse or see this variable outside of the function.
  console.log(viewed);
...
function handleClick(event) {
  ...
  for(let i = 0; i < names.length; i++){
    if(event.target.id === allProducts[i].name) {
      ...
      console.log(`${vent.target.id} has  ${allProducts[i].votes} votes in ${ allProducts[i].views} views`);
    }
  }
